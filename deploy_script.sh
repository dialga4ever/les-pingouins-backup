...
# Ce script sera exécuté une fois la dernière version de votre code récupérée depuis GitLab.i
...
# echo "Redémarrage du serveur pm2 \`server-Les Pingouins\`"
# # Needs sudo because server has been started as root
# sudo /custom-git-hooks/.nvm/versions/node/v16.13.0/bin/pm2 --silent restart "server-Les Pingouins" || fail "Redémarrage du serveur échoué "
